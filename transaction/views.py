...
from .constants import DEPOSIT, WITHDRAWAL, LOAN, LOAN_PAID, SEND_MONEY
...
class WithdrawMoneyView(TransactionCreateMixin):
    ...
    def form_valid(self, form):
        amount = form.cleaned_data.get('amount')
        bank = BankInfo.objects.get(name=NAME)
        
        if bank.is_bankcrupt :
            messages.warning(
            self.request,
            f'Bank is bankcrupt'
            )
...
class SendMoneyView(LoginRequiredMixin, CreateView):
    ...
    def form_valid(self, form):
        ...
        account2 = UserBankAccount.objects.get(account_no=send_to)
        account = self.request.user.account
        
        if account2 or account2 != account:
            account.balance -= amount # amount = 200, tar ager balance = 0 taka new balance = 0+200 = 200
            ...
            messages.success(
                self.request,
                f'{"{:,.2f}".format(float(amount))}$ was send to {account2.account_no} successfully'
            )
            return super().form_valid(form)
